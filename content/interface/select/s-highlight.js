...
function init() {
    ...
    const setPaddingPosition = (box, tRect, padding) => {
    ...
    const setMarginPosition = (box, tRect, margin) => {
    ...
    const setBodyPosition = (box, tRect, padding) => {
        // -2px height/width to account for borders
    ...
    function trackFrame() {
        if (select.object) {
            ...
            if (padding.length < 4)
                padding = [...padding, ...new Array(4 - padding.length).fill(0)];
            let margin = css.margin.replace(/px/g, '').split(' ').map(el => parseFloat(el));
            if (margin.length < 4)
                margin = [...margin, ...new Array(4 - margin.length).fill(0)];
