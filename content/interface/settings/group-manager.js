...
function init() {
    ...
    window.addEventListener('resize', () => {
        ...
        if (rect.right > window.innerWidth)
            menuElement.style.left = `${window.innerWidth - rect.width}px`;
        if (rect.bottom > window.innerHeight)
            menuElement.style.top = `${window.innerHeight - rect.height}px`;
        if (rect.left < 0)
