...
function main() {
    ...
    document.addEventListener('contextmenu', e => {
        if (e.altKey && !e.target.classList.contains(cst.prefix)) {
            e.preventDefault();
            let x = e.clientX;
            let y = e.clientY;
            const c_rect = ctx.menu.getBoundingClientRect();
            if (x + c_rect.width > window.innerWidth)
                x -= c_rect.width;
            if (y + c_rect.height > window.innerHeight)
                y -= c_rect.height;
            ctx.menu.style.left = `${x}px`;
            ctx.menu.style.top = `${y}px`;
            ctx.setTarget(e.target);
            doc.t(ctx.menu).addClass('shown');
        }
    });
    window.addEventListener('click', e => {
        if (!doc.isUiPart(e.target))
            doc.t(ctx.menu).removeClass('shown');
    });
    document.body.addEventListener('mouseover', e => {
        gline.target = e.target;
    });
    const setBoxPosition = (box, tRect, styles) => {
    ...
    const glineRefresh = () => {
        if (doc.isDOM(gline.target)) {
            if (!gline.target.classList.contains(cst.prefix)) {
                ...
                if (padding.length < 4)
                    padding = [...padding, ...new Array(4 - padding.length).fill(0)]; // make sure all 4 padding values are present
                let margin = tStyle.margin.replace(/px/g, '').split(' ').map(el => parseFloat(el));
                if (margin.length < 4)
                    margin = [...margin, ...new Array(4 - margin.length).fill(0)]; // make sure all 4 margin values are present
...
export { main as initSelectionManagement, gline };
